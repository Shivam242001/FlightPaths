...
public class CS400Graph<T> implements GraphADT<T> {
    ...
    protected class Path implements Comparable<Path> {
        ...
        public Path(Path copyPath, Edge extendBy) {
            ...
            for (T vertex : copyPath.dataSequence) {
                this.dataSequence.add(vertex);
            }
    ...
    protected Path dijkstrasShortestPath(T start, T end) {
        ...
        // intializes the priority queue, and an array list that will store all the
        // vertices visited
        PriorityQueue<Path> queue = new PriorityQueue<Path>();
        List<Vertex> visited = new ArrayList<Vertex>();

        // initializes variables that store the source and target nodes for the data
        // given in the
        // function parameters
        Vertex sourceNode;
        Vertex targetNode;

        // checks if the start data given is valid
        try {
            sourceNode = vertices.get(start);
        } catch (Exception e) {
            throw new NoSuchElementException("No vertex with the given start data can be found.");
        }

        // checks if the end data given is valid
        try {
            targetNode = vertices.get(end);
        } catch (Exception e) {
            throw new NoSuchElementException("No vertex with the given end data can be found.");
        }

        // ensures that the non-existent vertex case is handled for the start node
        if (sourceNode == null) {
            throw new NoSuchElementException("No vertex with the given start data can be found.");
        }

        // ensures that the non-existent vertex case is handled for the end node
        if (targetNode == null) {
            throw new NoSuchElementException("No vertex with the given end data can be found.");
        }

        // checks if source node == target node
        if (sourceNode == targetNode) {
            return new Path(sourceNode);
        }

        // initializes a base path that is stored as the initial element of the queue
        Path path = new Path(sourceNode);
        queue.add(path);

        // initializes an array list that stores all the paths from the source to target
        // node
        List<Path> pathsFound = new ArrayList<Path>();

        // looping through all possible paths to the target node
        while (!queue.isEmpty()) {
            Path curPath = queue.poll();
            sourceNode = curPath.end;

            // all the edges from the current node at the front of the frontier
            LinkedList<Edge> edgesLeaving = sourceNode.edgesLeaving;

            // adds all the unvisited nodes and paths to the frontier
            for (Edge edge : edgesLeaving) {
                if (!visited.contains(edge.target)) {
                    Path newPath = new Path(curPath, edge);
                    queue.add(newPath);

                    // any path from the source to the target is added to the arraylist
            ...
            // explored nodes are added to the visited arraylist
            visited.add(sourceNode);
        }

        // finds the path from the source to the target with the minimum distance, i.e,
        // shortest
        ...
        if (shortestPath == null) {
            throw new NoSuchElementException("There exists no path between the give nodes.");
        }
